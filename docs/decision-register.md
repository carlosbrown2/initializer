...
| Confidence | post-agent routing on BEAD_DONE | derived from observable signals (bash-observed gate result, commit diff lines, whether scripts/hooks/ or CLAUDE.md was touched, retry count) — not an agent self-grade. Non-PASS gate is terminal LOW; each risk axis downgrades one level (HIGH→MEDIUM→LOW floor) | scripts/ralph/lib.sh compute_confidence is sourced by scripts/ralph/ralph.sh and called on BEAD_DONE with signals gathered from git-show HEAD and the pre-iteration fail-count snapshot; scripts/ralph/lib.sh read_auto_land_policy / should_auto_land route the verdict against the `auto-land:` policy in CLAUDE.md; tests/hooks/ralph.bats covers gate-FAIL/SKIPPED/empty terminal LOW, each downgrade axis in isolation, the boundary at diff=500, two-axis LOW, three-axis LOW floor, and defaults for omitted trailing args | bounded |
